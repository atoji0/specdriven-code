<!--
  ============================================================
  入力画面サンプル（画面区分: 入力（基本）・ダイアログ版）
  ファイル名: list-normal/EditDialog.vue
  ...
  実装パターン一覧:
    テキスト        → codeValue（新規時のみ入力可）、nameValue
    数値        → numericValue（3桁カンマ表示）
    日付        → dateValue（YYYY/MM/DD）
    固定値選択  → selectValue
    マスタ参照  → masterValue + linkedValue（連動）
    フラグ      → flagValue（0=OFF / 1=ON）
    テキストエリア → memoValue
...
const dialogTitle = computed(() => (isEditMode.value ? "入力画面タイトル（変更）" : "入力画面タイトル（新規登録）"));
