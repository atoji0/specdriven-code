<!--
  ============================================================
  一覧画面サンプル（画面区分: 入力（マスタ選択ダイアログ版））
  ファイル名: select/List.vue
  ============================================================

  画面構成:
    - データ一覧（紐付きマスタ名を表示）
    - [選択] → EditDialog.vue
        - 親データ選択: MasterSelectDialog（単一選択）
        - 子マスタ選択: MasterMultiSelectDialog（複数選択）
    - [新規作成] → 新規モードで EditDialog.vue を開く

  input-parent-child-select との違い:
    - 検索パネル＋チェックボックステーブルの代わりに
      MasterSelectDialog / MasterMultiSelectDialog コンポーネントを使用
    - 「ダイアログで検索して選択」する UX に変更
    ...
    <el-collapse model-value="result">
      <el-collapse-item name="result">
        ...
        <el-table :data="pagedItems" stripe class="mt-4">
          ...
          <!-- 選択済みマスタ（1:N・サロゲートキー結合） -->
          <el-table-column label="選択マスタ" min-width="260">
